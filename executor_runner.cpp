...
DEFINE_string(weight_path, "weight.pte", "Weight in pte format.");

DEFINE_string(export_path, "export.bin", "export data in binary format.");
...
int main(int argc, char **argv) {
  ...
  const char *weight_path = FLAGS_weight_path.c_str();
  ...
  ET_CHECK_MSG(method_meta.ok(), "Failed to get method_meta for %s: 0x%x",
  ...
  ET_LOG(Info, "Using custom executor runner ");
  ET_LOG(Info, "Using float as default ! ");
  // idx being 0
  ...
  ET_CHECK_MSG(file_size == expected_size,
               "Input size mismatch. file bytes: %zu, tensor bytes: %zu",
  ...
  // index = 0
  const char *output_file_name = FLAGS_export_path.c_str();
  auto output_tensor = outputs[0].toTensor();
  ET_LOG(Info, "write %zu bytes to %s", output_tensor.nbytes(),
         output_file_name);

  std::ofstream fout(output_file_name, std::ios::binary);
  ...
  if (prof_result.num_bytes != 0) {
    std::cout << "Profiling data written to " << FLAGS_prof_result_path.c_str()
              << "\n";
...
/* hint for multiple input with input list */
/* similar output format could be obtain by the same method*/
/*
